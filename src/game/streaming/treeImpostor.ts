...
/**
 * Tree impostors — billboard LOD for distant trees (E5.1, FLO-394).
 *
 ...
 *
 * Out of scope (later tickets): hysteresis / cross-fade to hide the swap pop
 * (E5.2, FLO-393) and thin-instancing the forest (E5.3). The single swap pop
 * here is documented-acceptable; a cheap mitigation is a small overlap band,
 * which E5.2 owns.
 */
...
export function attachTreeImpostor(
  ...
  for (const m of sources) {
    if (m !== primary) m.addLODLevel(swapDistance, null)
  }
