...
- **E2.1 Health & damage model** `[x]` — **FLO-308** ✅ merged aa65342 (PR #23) — pure `src/game/health` model (HP, `applyDamage`, `isAlive`); **FLO-310** ✅ (PR #31) completed the full player life-cycle on top of it: `healthSlice` `isAlive`/`lastDamageAt` + typed `applyPlayerDamage({amount,source,kind})` funnel, dedicated `dead` app phase with frozen input + **Respawn**, HUD HP bar, dev-only debug damage (K / `window.korovanyDamage`), and HP save round-trip. See [health-system.md](../guide/health-system.md).
