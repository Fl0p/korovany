...
- **P7.4 Player-character & ground feel** `[x]` ✅ — **[FLO-422](/FLO/issues/FLO-422)** (Iris, done, `e54b4b7`) — **procedural flat-shaded fighter avatar** (`playerAvatar.ts`, `buildPlayerAvatar`) in a boxer's guard + faceted low-poly ground (`worldBounds.ts`). The static GLB hero (FLO-434 repose) **could not animate** (no skeleton), so it is retired to reference-only and the live player visual is built in code — a two-way door for a future rigged hero behind the same `root`+animator contract. Supersedes the interim FLO-420 (`371b715`, olive matte ground) and FLO-434 (`b901a78`, GLB repose). See `docs/guide/procedural-animation.md`.
